...
#### WSL
Télécharger le fichier helios.tar

Ouvre une invite de commande

Importer le fichier source
```sh
wsl --import Helios "C:\wsl\Helios" "C:\wsl\temp_helios\helios.tar"
```

Créer un utilisateur
```sh
wsl -u root -d Helios -- adduser adel
```

Donner les droits administrateur à l'utilisateur
```sh
wsl -u root -d Helios -- usermod -aG sudo adel
```

Définir l'utilisateur comme utilisateur par défaut
```sh
wsl -d Helios -u root -- sh -c "echo '[user]\ndefault=adel' > /etc/wsl.conf"
```

Configurer l'utilisateur git
```sh
git config --global user.email "*******@cat-amania.com"
