...
def ajoute_les_autorisations_des_établissements_médico_sociaux(chemin_du_fichier: str, base_de_données: Engine, logger: Logger) -> None:
    ...
    with base_de_données.begin() as connection:
        connection.execute(f"DELETE FROM {TABLES_DES_AUTORISATIONS_DES_ÉTABLISSEMENTS_MÉDICO_SOCIAUX};")
        logger.info("[FINESS] Anciennes autorisations médico-sociales supprimées")
        sauvegarde(connection, TABLES_DES_AUTORISATIONS_DES_ÉTABLISSEMENTS_MÉDICO_SOCIAUX, autorisations_des_établissements_médico_sociaux)
        mets_à_jour_la_date_de_mise_à_jour_du_fichier_source(connection, date_du_fichier_des_autorisations, FichierSource.FINESS_CS1400105)
    logger.info(f"[FINESS] {autorisations_des_établissements_médico_sociaux.shape[0]} autorisations médico-sociales sauvegardées")
