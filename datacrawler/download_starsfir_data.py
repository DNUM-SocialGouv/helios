...
def list_starsfir_files(ftp_host: ftputil.FTPHost, remote_path: str) -> list[str]:
...
def main() -> None:
    """Script pour télécharger les données brutes STARSFIR depuis un serveur FTPS."""
    ...
    try:
        ...
        with ftputil.FTPHost(ftps_host, ftps_username, ftps_password, session_factory=_make_session_factory(ftps_port)) as ftp_host:
            ...
            try:
                filenames = list_starsfir_files(ftp_host, remote_path)
            except ftputil.error.FTPError as exc:
                logger.exception("Impossible de lister les fichiers sur le FTPS: %s", exc)
                return

            if not filenames:
                logger.warning("Aucun fichier STARSFIR correspondant trouvé sur le FTPS.")
                return
