...
from datacrawler.transform.équivalences_finess_helios import XPATH_FINESS_CS1500106, type_des_colonnes_categories_finess

def import_ref_categories(chemin_du_fichier: str, base_de_donnees: Engine, logger: Logger) -> None:
    logger.info("[FINESS] Récupère les catégories")
    ...
    with base_de_donnees.begin() as connection:
        écrase_et_sauvegarde_les_données_avec_leur_date_de_mise_à_jour(
            nom_de_la_donnée="réferentiel des catégories finess",
            fournisseur="FINESS",
            connection=connection,
            table=TABLE_REF_CATEGORIES,
            données=categies_finess,
            logger=logger,
            fichiers_mis_à_jour=[],
        )
