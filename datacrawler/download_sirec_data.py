...
def connect_sftp(host: str, port: int, username: str, key_filename: str, logger: Logger) -> Tuple[paramiko.SSHClient, paramiko.SFTPClient]:
...
def list_sirec_files(sftp: paramiko.SFTPClient, remote_path: str, file_prefix: str) -> list:
...
def _safe_close(sftp: Optional[paramiko.SFTPClient], ssh: Optional[paramiko.SSHClient], logger: Logger) -> None:
    try:
        ...
        logger.debug("Une erreur est survenue lors de la fermeture de la connexion au sftp: %s", exc)


def main() -> None:
    """Script pour télécharger les données brutes SIREC depuis un serveur SFTP.
    ...
    le fichier le plus récent avec le préfixe 'sirec_', le télécharge dans
    ...
    remote_path = "SIREC"
    local_path = Path(variables_d_environnement["SIREC_DATA_PATH"])
    file_prefix = "sirec_"
    ...
    try:
        ...
        try:
            entries = list_sirec_files(sftp, remote_path, file_prefix)
