...
if __name__ == "__main__":
    ...
    chemin_local_du_fichier_profession_groupe = os.path.join(
        vegie_rh_data_path,
        trouve_le_nom_du_fichier(fichiers, FichierSource.PREFIX_FICHIER_PROFESSION_GROUPE.value, logger_helios)
    )
    chemin_local_du_fichier_ref_masque = os.path.join(
        vegie_rh_data_path,
        trouve_le_nom_du_fichier(fichiers, FichierSource.PREFIX_FICHIER_REF_MASQUE.value, logger_helios)
    )
    chemin_local_du_fichier_ref_profession_groupe = os.path.join(
        vegie_rh_data_path,
        trouve_le_nom_du_fichier(fichiers, FichierSource.PREFIX_FICHIER_REF_PROFESSION_GROUPE.value, logger_helios)
    )
    chemin_local_du_fichier_ref_qualite = os.path.join(
        vegie_rh_data_path,
        trouve_le_nom_du_fichier(fichiers, FichierSource.PREFIX_FICHIER_REF_QUALITE.value, logger_helios)
    )
    chemin_local_du_fichier_ref_redressement = os.path.join(
        vegie_rh_data_path,
        trouve_le_nom_du_fichier(fichiers, FichierSource.PREFIX_FICHIER_REF_REDRESSEMENT.value, logger_helios)
    )
    ...
    inserer_nouvelles_donnees(Table.PROFESSION_GROUPE.value, base_de_données, SOURCE, df_filtré, logger_helios)
