...
class TestImportVigiePprofessionGroupe:
    ...
    def _setup_file_paths(self, vegie_rh_data_path: str) -> Dict[str, str]:
        ...
        return {
            "profession_groupe": os.path.join(
                vegie_rh_data_path, trouve_le_nom_du_fichier(fichiers, FichierSource.VIGIE_RH_PROFESSION_GROUPE.value, mocked_logger)
            ),
            "ref_profession_groupe": os.path.join(
                vegie_rh_data_path, trouve_le_nom_du_fichier(fichiers, FichierSource.VIGIE_RH_REF_PROFESSION_GROUPE.value, mocked_logger)
            ),
            "table_passage_profession": os.path.join(
                vegie_rh_data_path, trouve_le_nom_du_fichier(fichiers, FichierSource.VIGIE_RH_REF_PASSAGE_GROUPE_FILIERE.value, mocked_logger)
            ),
        }
    ...
    def _read_and_assert_dataframes(self, file_paths: Dict[str, str]) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        df_passage_profession = lis_le_fichier_parquet(file_paths["table_passage_profession"], ColumMapping.PASSAGE_GROUPE_FILIERE.value)
    ...
    def _insert_new_data(self, data_frames: Tuple[pd.DataFrame, pd.DataFrame], dates: Dict[str, str]) -> None:
        with base_de_données_test.begin() as connection:
            ...
            inserer_nouvelles_donnees(
                TABLE_PROFESSION_GROUPE, connection, SOURCE, data_frames[1], mocked_logger, FichierSource.VIGIE_RH_PROFESSION_GROUPE, dates["profession_groupe"]
            )
