...
import datacrawler
from datacrawler.load.nom_des_tables import TABLES_DES_RESSOURCES_HUMAINES_ENTITE_JURIDIQUE, TABLE_DES_MISES_À_JOUR_DES_FICHIERS_SOURCES, FichierSource
from datacrawler.test_helpers import (
    ...
    sauvegarde_les_indicateurs_ressources_humaines_en_base_entite_juridique,
)

from datacrawler.test_helpers.helios_builder import helios_entite_juridique_ressources_humaines_builder

from datacrawler.ajoute_le_bloc_ressources_humaines_des_entites_juridiques import ajoute_le_bloc_ressources_humaines_des_entite_juridiques
...
class TestAjouteBlocRessourcesHumainesEntiteJuridique:
    ...
    def test_savegarde_les_donnees_des_ressources_humaines(self) -> None:
        ...
        ajoute_le_bloc_ressources_humaines_des_entite_juridiques(chemin_du_fichier_quo_san_finance, base_de_données_test, mocked_logger)
        ...
        donnees_ressources_humaines_enregistrees = pd.read_sql(TABLES_DES_RESSOURCES_HUMAINES_ENTITE_JURIDIQUE, base_de_données_test)
    ...
    def test_sauvegarde_les_dates_de_mise_a_jour_des_indicateurs_ressources_humaines(self) -> None:
        ...
        date_mise_a_jour_fichier_quo_san_finance = base_de_données_test.connect().execute(
    ...
    def test_supression_donnees_existantes_avant_insertion_des_nouvelles(self) -> None:
        ...
        donnees_avant_suppression_attendu = pd.DataFrame([helios_entite_juridique_ressources_humaines_builder()])
        sauvegarde_les_indicateurs_ressources_humaines_en_base_entite_juridique(donnees_avant_suppression_attendu, base_de_données_test)
        donnees_avant_suppression = pd.read_sql(TABLES_DES_RESSOURCES_HUMAINES_ENTITE_JURIDIQUE, base_de_données_test)
    ...
    def test_revient_a_la_situation_initiale_si_l_ecriture_des_indicateurs_ressources_humaines_echoue(self, mocked_sauvegarde: Mock) -> None:
        ...
        donnees_table_ressources_humaines_entite_juridique = pd.DataFrame([helios_entite_juridique_ressources_humaines_builder()])
        sauvegarde_les_indicateurs_ressources_humaines_en_base_entite_juridique(donnees_table_ressources_humaines_entite_juridique, base_de_données_test)
        ...
        pd.testing.assert_frame_equal(
            donnees_ressources_humaines_enregistrees.sort_index(axis=1), donnees_table_ressources_humaines_entite_juridique.sort_index(axis=1)
        )

        date_du_fichier_aquo_san_finance = base_de_données_test.connect().execute(
            text(f"""SELECT * FROM {TABLE_DES_MISES_À_JOUR_DES_FICHIERS_SOURCES} WHERE fichier = '{FichierSource.DIAMANT_QUO_SAN_FINANCE.value}'""")
        )
