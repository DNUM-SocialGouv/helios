...
def filtrer_les_donnees_pyramide(donnees: pd.DataFrame, base_de_donnees: Engine) -> pd.DataFrame:
    ...
    donnees_filtrées = donnees[
        ...
        (donnees["numero_finess_etablissement_territorial"].astype(str).isin(numéros_finess_liste))
    ]

    return donnees_filtrées

def import_donnees_pyramide(chemin_local_du_fichier_ref: str, chemin_local_du_fichier_donnees: str, base_de_données: Engine, logger: Logger) -> None:
    ...
    if date_du_fichier_vigierh_ref_tranche_age != date_du_fichier_vigierh_donnees_pyramide:
        ...
        traite_ref = verifie_si_le_fichier_est_traite(date_du_fichier_vigierh_ref_tranche_age, base_de_données, FichierSource.VIGIE_RH_REF_TRANCHE_AGE.value)
        traite_donnees = verifie_si_le_fichier_est_traite(date_du_fichier_vigierh_donnees_pyramide, base_de_données, FichierSource.VIGIE_RH_PYRAMIDE.value)
        if traite_ref & traite_donnees:
            ...
            donnees_pyramide_filtrees = filtrer_les_donnees_pyramide(donnees_pyramide, base_de_données)
            supprimer_donnees_existantes(TABLE_TRANCHE_AGE, base_de_données, SOURCE, logger)
