...
from datacrawler.transform.équivalences_finess_helios import colonnes_à_garder_finess_cs1400105, index_des_autorisations, équivalences_finess_cs1400105_helios
...
def transforme_les_données_finess_cs1400105(
    ...
    logger.info(f"[FINESS] {est_dans_finess.sum()} activités sont liées à un ET trouvé en base dans le fichier finess_cs1400105")
    ...
    autorisations_toujours_autorisées = (
        ...
        .dropna(subset=index_des_autorisations)
    )

    return détermine_l_installation_de_l_autorisation(autorisations_toujours_autorisées).set_index(index_des_autorisations)
