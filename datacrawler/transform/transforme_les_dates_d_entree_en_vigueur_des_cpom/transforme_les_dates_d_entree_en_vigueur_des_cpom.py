...
def formate_la_date_d_entrée_en_vigueur_des_cpom(données_des_dates_des_cpom: pd.DataFrame) -> pd.DataFrame:
    def formate_la_date(date_du_cpom: str) -> str:
        return datetime.strptime(date_du_cpom, "%d/%m/%Y").strftime("%Y-%m-%d")

    dates_formatées_des_entrés_des_cpom = données_des_dates_des_cpom.copy()
    dates_formatées_des_entrés_des_cpom["date_d_entree_en_vigueur"] = données_des_dates_des_cpom["date_d_entree_en_vigueur"].map(
        formate_la_date, na_action="ignore"
    )
    return dates_formatées_des_entrés_des_cpom


def transforme_les_dates_d_entrée_en_vigueur_des_cpom(données_ann_ms_tdp_et: pd.DataFrame, numéros_finess_connus: pd.DataFrame, logger: Logger) -> pd.DataFrame:
    ...
    dates_d_entrée_en_vigueur_des_cpom = (
    ...
    return formate_la_date_d_entrée_en_vigueur_des_cpom(dates_d_entrée_en_vigueur_des_cpom).set_index(index_des_dates_d_entrée_en_vigueur_des_cpom)
