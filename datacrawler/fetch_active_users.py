...
def get_active_users(engine: Engine) -> pd.DataFrame:
    query = """
        SELECT 
            r.role_code AS role_code,
            COUNT(DISTINCT u.ut_id) AS active_users
        FROM utilisateur u
...
if __name__ == "__main__":
    ...
    active_users_by_role = get_active_users(base_de_données_helios)
    # Send Matomo event for each role (except admin national)
    for _, row in active_users_by_role.iterrows():
        ...
        if mapped_role_level:
            ...
            logger_helios.info(f"Event sent for {role_code} ({mapped_role_level}): {int(row['active_users'])} users")
        else:
