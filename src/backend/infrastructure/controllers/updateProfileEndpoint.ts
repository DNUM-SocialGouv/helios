...
export async function updateProfileEndpoint(dependencies: Dependencies, code: string, value: ProfileValue): Promise<void> {
    try {
        const ProfileUseCase = new ProfilesUseCase(dependencies.profileLoader);
        return await ProfileUseCase.updateProfile(code, value);
    } catch (error) {
