...
import { fakeBackendDependencies, fakeLogger } from '../../../testHelperBackend'
...
describe('Téléchargement d’une source de données via un SFTP', () => {
  ...
  it('créer un répertoire au nom de la source de données avant de la télécharger', async () => {
    // GIVEN
    setup()
    const sftpDownloadDataSource = new SftpDownloadRawData(fakeBackendDependencies.environmentVariables, fakeLogger)
  ...
  it('se connecter au SFTP, télécharger un répertoire et se déconnecter', async () => {
    ...
    expect(Client.prototype.downloadDir).toHaveBeenCalledWith(sftpPath, `${fakeBackendDependencies.environmentVariables.SFTP_LOCAL_PATH}/${localPath}`)
