...
describe('Décompression d’une source de données', () => {
  it('décompresser une source de données avec la commande gunzip', async () => {
    ...
    const gunzipUnzipDataSource = new GunzipUnzipRawData(fakeBackendDependencies.environmentVariables, fakeLogger)

    // WHEN
    gunzipUnzipDataSource.handle(dataSource, localPath)

    // THEN
    expect(child.execSync).toHaveBeenCalledWith(`gunzip -rf ${fakeBackendDependencies.environmentVariables.SFTP_LOCAL_PATH}/${localPath}`)
