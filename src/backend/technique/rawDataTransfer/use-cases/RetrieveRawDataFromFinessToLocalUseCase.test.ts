import { fakeBackendDependencies } from '../../testHelperBackend'
import { RetrieveRawDataFromFinessToLocalUseCase } from './RetrieveRawDataFromFinessToLocalUseCase'

describe('Récupération des sources de données FINESS en local', () => {
  it('récupérer les sources de données FINESS en local', async () => {
    // GIVEN
    const dataSource = 'FINESS'
    const sftpPath = 'flux_finess'
    const retrieveDataSourceFromFinessToLocal = new RetrieveRawDataFromFinessToLocalUseCase(
      fakeBackendDependencies.downloadRawData,
      fakeBackendDependencies.unzipRawData
    )

    // WHEN
    await retrieveDataSourceFromFinessToLocal.handle()

    // THEN
    await expect(fakeBackendDependencies.downloadRawData.handle).toHaveBeenCalledWith(dataSource, sftpPath, 'finess')
    await expect(fakeBackendDependencies.unzipRawData.handle).toHaveBeenCalledWith(dataSource, 'finess')
  })

  it('une erreur est survenue lors du téléchargement des données', async () => {
    // GIVEN
    const errorMessage = 'téléchargement interrompu'
    jest.spyOn(fakeBackendDependencies.downloadRawData, 'handle').mockImplementation(jest.fn(async () => {
      await Promise.reject(new Error(errorMessage))
    }))
    const retrieveDataSourceFromFinessToLocal = new RetrieveRawDataFromFinessToLocalUseCase(
      fakeBackendDependencies.downloadRawData,
      fakeBackendDependencies.unzipRawData
    )

    try {
      // WHEN
      await retrieveDataSourceFromFinessToLocal.handle()
      throw new Error('ne devrait pas passer ici')
    } catch (error) {
      // THEN
      expect(error.message).toBe(`[Helios] ${errorMessage}`)
    }
  })

  it('une erreur est survenue lors du la décompression des fichiers', async () => {
    // GIVEN
    const errorMessage = 'décompression interrompue'
    jest.spyOn(fakeBackendDependencies.unzipRawData, 'handle').mockImplementation(jest.fn(async () => {
      await Promise.reject(new Error(errorMessage))
    }))
    const retrieveDataSourceFromFinessToLocal = new RetrieveRawDataFromFinessToLocalUseCase(
      fakeBackendDependencies.downloadRawData,
      fakeBackendDependencies.unzipRawData
    )

    try {
      // WHEN
      await retrieveDataSourceFromFinessToLocal.handle()
      throw new Error('ne devrait pas passer ici')
    } catch (error) {
      // THEN
      expect(error.message).toBe(`[Helios] ${errorMessage}`)
    }
  })
})
