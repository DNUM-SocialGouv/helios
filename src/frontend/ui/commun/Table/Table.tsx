      ...
      {data.map((row, rowIndex) => (
        <tr data-row-key={rowIndex} id={`table-selectable-row-key-${rowIndex}`} key={rowIndex}>
          ...
          {headers.map((header, colIndex) => (
            <td className={`${isCenter || header.key === "favori" ? "fr-cell--center" : styles["cell-container"]} ${(row as any)[header.key] === 'Consultation non autorisé' ? styles["cell-not-authorized"] : ''}`} key={colIndex}>
              {header.key === "delete" && (
                <button
