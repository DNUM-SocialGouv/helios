...
describe("La page établissement territorial - bloc identité", () => {
  ...
  it("affiche l’adresse", () => {
    ...
    const adresse = within(indicateurs[3]).getByText("1 RTE DE VEYZIAT 01117 OYONNAX CEDEX", { selector: "p" });
  ...
  it("affiche le téléphone et e-mail", () => {
    ...
    const téléphoneEtEmail = within(indicateurs[4]).getByText("01 23 45 67 89 | a@example.com", { selector: "p" });
  ...
  it("affiche un lien pour naviguer vers l’entité juridique de rattachement", () => {
    ...
    const entitéJuridiqueDeRattachement = within(indicateurs[5]).getByRole("link", { name: "EJ - 010008407 - CH DU HAUT BUGEY" });
  ...
  it("affiche la catégorie de l’établissement avec son libellé", () => {
    ...
    const catégorieDeLÉtablissement = within(indicateurs[6]).getByText("300 - Ecoles Formant aux Professions Sanitaires", { selector: "p" });
  ...
  it("affiche le mode de tarification", () => {
    ...
    const modeDeTarification = within(indicateurs[7]).getByText("03 - ARS établissements Publics de santé dotation globale", { selector: "p" });
  ...
  it("affiche l’indicateur de mono-établissement à oui quand il est tout seul dans l’entité juridique", () => {
    ...
    // WHEN
    renderFakeComponent(<SessionProvider session={mockSession}><PageÉtablissementTerritorialMédicoSocial rechercheViewModel={rechercheViewModel} établissementTerritorialViewModel={établissementTerritorialMonoÉtablissement} /> </SessionProvider>);

    // THEN
    const ficheDIdentité = screen.getByRole("region", { name: wording.TITRE_BLOC_IDENTITÉ });
    const indicateurs = within(ficheDIdentité).getAllByRole("listitem");
    const libelléMonoÉtablissement = within(indicateurs[9]).getByText(
    ...
    const monoÉtablissement = within(indicateurs[9]).getByText(wording.OUI);
  ...
  it("affiche l’indicateur de mono-établissement à non quand il n’est pas tout seul dans l’EJ", () => {
    ...
    expect(libelléMonoÉtablissement).toBeInTheDocument();
    const monoÉtablissement = within(indicateurs[9]).getByText(wording.NON);
    expect(monoÉtablissement).toBeInTheDocument();
  });
  ...
  describe("l’indicateur d’établissement principal ou secondaire", () => {
    it('affiche "Principal" si l’établissement est un établissement principal', () => {
      ...
      // WHEN
      renderFakeComponent(<SessionProvider session={mockSession}><PageÉtablissementTerritorialMédicoSocial rechercheViewModel={rechercheViewModel} établissementTerritorialViewModel={établissementTerritorialPrincipal} /> </SessionProvider>);

      // THEN
      const ficheDIdentité = screen.getByRole("region", { name: wording.TITRE_BLOC_IDENTITÉ });
      const indicateurs = within(ficheDIdentité).getAllByRole("listitem");
      const libelléÉtablissementPrincipalOuSecondaire = within(indicateurs[10]).getByText(
      ...
      const établissementPrincipalOuSecondaire = within(indicateurs[10]).getByText(wording.PRINCIPAL);
    ...
    it('affiche "Secondaire" et le numéro Finess de l’établissement principal si l’établissement n’est pas un établissement principal', () => {
      ...
      expect(libelléÉtablissementPrincipalOuSecondaire).toBeInTheDocument();
      const établissementPrincipalOuSecondaire = within(indicateurs[10]).getByText(`${wording.SECONDAIRE} (${wording.PRINCIPAL} : 010005239)`);
      expect(établissementPrincipalOuSecondaire).toBeInTheDocument();
    });
  });
  ...
  it("affiche la date d’entrée en vigueur du CPOM", () => {
    ...
    const ficheDIdentité = screen.getByRole("region", { name: wording.TITRE_BLOC_IDENTITÉ });
    const indicateurs = within(ficheDIdentité).getAllByRole("listitem");
    const libelléDateDEntréeEnVigueurDuCpom = within(indicateurs[11]).getByText(
    ...
    const indicateurÀVenir = within(indicateurs[11]).getByText("01/01/2021");
  ...
  describe('affiche "non renseigné" quand une valeur est vide', () => {
    it("pour le téléphone", () => {
      ...
      // WHEN
      renderFakeComponent(<SessionProvider session={mockSession}><PageÉtablissementTerritorialMédicoSocial rechercheViewModel={rechercheViewModel} établissementTerritorialViewModel={établissementTerritorialSansTéléphone} /> </SessionProvider>);
    ...
    it("pour l’e-mail", () => {
      ...
      // WHEN
      renderFakeComponent(<SessionProvider session={mockSession}><PageÉtablissementTerritorialMédicoSocial rechercheViewModel={rechercheViewModel} établissementTerritorialViewModel={établissementTerritorialSansEMail} /> </SessionProvider>);

      // THEN
      const ficheDIdentité = screen.getByRole("region", { name: wording.TITRE_BLOC_IDENTITÉ });
      const indicateurs = within(ficheDIdentité).getAllByRole("listitem");
      const téléphoneEtEmail = within(indicateurs[4]).getByText(`01 23 45 67 89 | ${wording.NON_RENSEIGNÉ}`, { selector: "p" });
      expect(téléphoneEtEmail).toBeInTheDocument();
    });
  ...
  it("affiche l’adresse incomplète lorsqu’il manque des champs d’adresse", () => {
    ...
    // WHEN
    renderFakeComponent(<SessionProvider session={mockSession}><PageÉtablissementTerritorialMédicoSocial rechercheViewModel={rechercheViewModel} établissementTerritorialViewModel={établissementTerritorialSansAdresseVoie} /></SessionProvider>);

    // THEN
    const ficheDIdentité = screen.getByRole("region", { name: wording.TITRE_BLOC_IDENTITÉ });
    const indicateurs = within(ficheDIdentité).getAllByRole("listitem");
    const adresseIncomplète = within(indicateurs[3]).getByText("1 RTE 01117 OYONNAX CEDEX", { selector: "p" });
