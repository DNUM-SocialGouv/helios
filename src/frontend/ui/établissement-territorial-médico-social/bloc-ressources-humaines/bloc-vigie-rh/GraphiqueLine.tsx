    ...
    plugins: {
      legend: {
        ...
        onClick: (_e, item, legend) => {
          ...
          const key = (multiCategories ?? []).find((c) => c.categorie.charAt(0).toUpperCase() + c.categorie.slice(1) === label)?.categorie;
          if (!key) return;
    ...
    <div className={`${classContainer} `}>
      <div className={`${styles["chartLineDiv"]}`} style={{}}>
        ...
        <Transcription
          ...
          identifiants={[wording.EFFECTIFS_TOTAUX, ...(multiCategories ?? []).map((c) => c.categorie.charAt(0).toUpperCase() + c.categorie.slice(1))]}
