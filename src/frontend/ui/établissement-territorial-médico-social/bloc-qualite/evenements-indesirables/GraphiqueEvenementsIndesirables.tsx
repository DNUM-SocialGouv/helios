...
export const GraphiqueEvenementsIndesirables = ({ data, dateMiseAJour, annees }: GraphiqueEvenementsIndesirablesProps) => {
    ...
    useEffect(() => {
        setTotal(data[annéeEnCours][0].evenementsClotures.length + data[annéeEnCours][0].evenementsEncours.length
            + data[annéeEnCours][1].evenementsClotures.length + data[annéeEnCours][1].evenementsEncours.length)
    }, [annéeEnCours])
    ...
    return (
        ...
        >
            <>
                ...
                <EvenementsIndesirablesTagMultiNiveaux evenementsIndesirablesAssociesAuxSoins={data[annéeEnCours][0]} evenementsIndesirablesDansET={data[annéeEnCours][1]} />
            </>
        </IndicateurGraphique>
    );
}