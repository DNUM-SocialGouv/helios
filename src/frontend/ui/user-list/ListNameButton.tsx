...
const ListNameButton = ({ id, name }: { id: number, name: string }) => {
  ...
  const duplicateExists = newName && userContext?.favorisLists?.some(list => list.id !== id && list.nom.trim().toLowerCase() === newName.trim().toLowerCase());
    ...
    {updateListName ?
      <>
        ...
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          {duplicateExists && <p className="fr-message fr-message--info" id="update-list-duplicate-message" style={{ margin: 0 }}>{wording.ETOILE_NOUVELLE_LISTE_EXISTANTE}</p>}
        </div>
      </> :
