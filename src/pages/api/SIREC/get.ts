...
function isValidFinessRpps(value: string): boolean {
  return /^\d{9}$/.test(value);
}

function containsCommaOrDotNumbers(row: Record<string, any>): boolean {
  const values = Object.values(row);
  return values.some((value) => typeof value === "string" && /^(\d+,\d+|\d+\.\d+)$/.test(value));
}

// Fonction pour vérifier si la ligne contient des valeurs numériques négatives
function containsNegativeNumbers(row: Record<string, any>): boolean {
  const values = Object.values(row);
      ...
      fs.createReadStream(filePath)
        ...
        .on("data", (row) => {
          ...
          if (!isValidColumnNames) {
            console.warn("Invalid column names:", row);
          ...
          if (!isValidYear(year)) {
            console.warn("Invalid year value for ANNEE_DE_RECEPTION:", row);
            return; // Ignorer la ligne
          }
          ...
          motifs.forEach((motif) => {
            if (!row.hasOwnProperty(motif)) {
              console.warn(`Missing data for motif ${motif} in row:`, row);
              return; // Ignorer la ligne
            }
          });
          ...
          if (!isValidFinessRpps(finessRpps)) {
            console.warn("Invalid value for NDEG_FINESS_RPPS:", row);
            return; // Ignorer la ligne
          }
