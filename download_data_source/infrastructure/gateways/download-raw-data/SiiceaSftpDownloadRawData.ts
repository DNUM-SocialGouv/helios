...
export class SiiceaSftpDownloadRawData implements DownloadRawData {
  constructor(
    ...
    private readonly prefixeDesFichiersATelecharger: string = "siicea_"
  ) { }
  ...
  async exécute(): Promise<void> {
    ...
    try {
      ...
      if (fichierATelecharger) {
        ...
        this.logger.error(`[SIICEA] Le fichier ${this.prefixeDesFichiersATelecharger} n’est pas présent sur le sftp.`);
      }
      ...
      throw new HeliosError(`[SIICEA] Une erreur est survenue lors de la connexion au SFTP : ${erreur.message}`);
    }
  }
  ...
  private async connexionAuSftp() {
    ...
    this.logger.info("[SIICEA] La connexion au SFTP est ouverte.");
  }
  ...
  private async deconnexionDuSftp() {
    ...
    this.logger.info("[SIICEA] La connexion au SFTP est fermée.");
  }
  ...
  private async telechargeLeFichier(fichierATelecharger: string) {
    ...
    this.logger.info(`[SIICEA] Le fichier SIICEA ${fichierATelecharger} a été téléchargé.`);
  }
  ...
  private trouveLeFichierLePlusRecentPortantLePrefixe(prefixeDesFichiersATelecharger: string, fichiersSurLeSftp: FileInfo[]) {
    ...
    this.logger.info(`[SIICEA] Le fichier SIICEA ${fichiersPertinentsTriesParDate} a été téléchargé.`);
