import { DomaineÉtablissementTerritorial } from "./DomaineÉtablissementTerritorial";

export type ÉtablissementTerritorialIdentité = Readonly<{
  adresseAcheminement: string;
  adresseNuméroVoie: string;
  adresseTypeVoie: string;
  adresseVoie: string;
  catégorieÉtablissement: string;
  codeModeTarification: string;
  commune: string;
  courriel: string;
  département: string;
  domaine: DomaineÉtablissementTerritorial;
  libelléCatégorieÉtablissement: string;
  libelléCourtCatégorieÉtablissement: string;
  classificationEtablissement?: Classification;
  libelléModeTarification: string;
  numéroFinessEntitéJuridique: string;
  numéroFinessÉtablissementPrincipal: string;
  numéroFinessÉtablissementTerritorial: string;
  raisonSociale: string;
  raisonSocialeCourte: string;
  siret: string;
  téléphone: string;
  typeÉtablissement: string;
  codeRégion: string;
  dateOuverture: string;
}>;

export enum Classification {
  PUBLICS_SITUATION_HANDICAP = "publics_en_situation_de_handicap",
  PERSONNES_AGEES = "personnes_agees",
  NON_CALSSIFIE = "non_classifie",
}

export const ListeDesCategoriesPourLaClassificationPublicsEnSituationHandicap = [
  'Service d’Education Spéciale et de Soins à Domicile',
  'Institut Médico Educatif',
  'Institut Thérapeutique Educatif et Pédagogique',
  'Etablissement pour Enfants ou Adolescents Polyhandicapés',
  'Centre Médico-Psycho-Pédagogique',
  'Centre Action Médico-Sociale Précoce',
  'Institut d’Education Motrice',
  'Institut pour Déficients Visuels',
  'Institut pour Déficients Auditifs',
  'Etablissement et Service de Pré-orientation',
  'Etablissement et Service d’Aide par le Travail',
  'Etablissement et Service de Réadaptation Professionnelle',
  'Maison d’Accueil Spécialisée',
  'Foyer d’Accueil Médicalisé pour adultes handicapés',
  'Service d’Accompagnement Medico-Social Adultes Handicapés',
  'Etablissement Accueil Médicalisé en tout ou partie personnes handicapés',
  'Centre de Ressources Sans Autre Indication',
  'Unités Evaluation Réentraînement et d’Orientation Sociale et Professionnelle'
];
export const ListeDesCategoriesPourLaClassificationPersonnesAgees = [
  'Etablissement d’Hébergement pour Personnes Agées Dépendantes',
  'Service de Soins Infirmiers A Domicile',
  'Service Autonomie Aide et Soins'
];
